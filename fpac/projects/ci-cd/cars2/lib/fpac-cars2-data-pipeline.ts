...
export class FpacCars2DataPipelineStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: EtlStackProps) {
    ...
    const projectDisplayName = props.project.toUpperCase();
    const projectsDir = `${rootDirPrefix}/${props.project}`;
    ...
    const projectS3Folders = new ProjectS3Folders(this, `${projectDisplayName}-ProjectS3Folders`, {
    ...
    const thirdPartyLayer = new lambda.LayerVersion(this, 'fpac-cdk-thirdparty-layer', {
      ...
      code: new AssetCode(`../shared/thirdparty-layers`),
    ...
    const step1GlueJob = new FpacGlueJob(this, `${projectDisplayName}-LandingFilesGlueJob`, {
      env: props.deployEnv,
      role: glueJobRole,
      scriptLocation: `${projectsDir}/glue/landingFiles/landing_job.py`,
      landingBucket: landingBucket.bucketName,
      cleanBucket: cleanBucket.bucketName,
      finalBucket: finalBucket.bucketName,
      jobType: `${projectDisplayName}-LandingFiles`,
      stepName: 'Step1',
      project: props.project,
    });
    ...
    const step2GlueJob = new FpacGlueJob(this, `${projectDisplayName}-CleansedFilesGlueJob`, {
      env: props.deployEnv,
      role: glueJobRole,
      scriptLocation: `${projectsDir}/glue/cleaningFiles/cleaning_job.py`,
      landingBucket: landingBucket.bucketName,
      cleanBucket: cleanBucket.bucketName,
      finalBucket: finalBucket.bucketName,
      jobType: `${projectDisplayName}-CleansedFiles`,
      stepName: 'Step2',
      project: props.project,
    });
    ...
    const step3GlueJob = new FpacGlueJob(this, `${projectDisplayName}-FinalFilesGlueJob`, {
      env: props.deployEnv,
      role: glueJobRole,
      scriptLocation: `${projectsDir}/glue/finalFiles/final_job.py`,
      landingBucket: landingBucket.bucketName,
      cleanBucket: cleanBucket.bucketName,
      finalBucket: finalBucket.bucketName,
      jobType: `${projectDisplayName}-FinalFiles`,
      stepName: 'Step3',
      project: props.project,
    });
    ...
    const crawlerName = `FSA-${props.deployEnv}-${projectDisplayName}-CRAWLER`;
    const crawler = new glue_l1.CfnCrawler(this, `${projectDisplayName}-ProcessedCrawler`, {
      name: crawlerName,
      role: crawlerRole.roleArn,
      databaseName: databaseName,
      targets: {
        s3Targets: [{
          path: `s3://${finalBucket.bucketName}/${props.project}/`,
        }],
      },
      schemaChangePolicy: {
    ...
    const crawlerTrigger = new glue_l1.CfnTrigger(this, `${projectDisplayName}-CrawlerTrigger`, {
      name: `${projectDisplayName}-${cdk.Stack.of(this).stackName}-crawler-trigger`,
      type: 'CONDITIONAL',
    ...
    // ===== Validator Lambda =====
    /*
    const validatorFn = new lambda.Function(this, `${projectDisplayName}-ValidatorFn`, {
      runtime: lambda.Runtime.NODEJS_20_X,
      functionName: `FSA-${props.deployEnv}-${projectDisplayName}-Validator`,
      handler: 'index.handler',
      code: lambda.Code.fromAsset(`${rootDirPrefix}/lambda/validate`),
      environment: { LANDING_BUCKET: landingBucket.bucketName, PROJECT: props.project },
      role: etlLambdaRole,
    });
    */
    const fpacLambdaConstruct = new FpacLambda(this, `${projectDisplayName}-ValidatorLambdaConstruct`, {
      functionName: `FSA-${props.deployEnv}-${projectDisplayName}-Validator`,
      functionCode: `${projectsDir}/lambda/validate`,
      role: etlLambdaRole,
      environment: {
        LANDING_BUCKET: landingBucket.bucketName,
        PROJECT: props.project,
      },
    });

    const validatorFn = fpacLambdaConstruct.lambdaFunction;
    landingBucket.grantRead(validatorFn);

    // ===== Step Functions: Validate → Run ETL → Start Crawler → Success/Fail =====
    const validateTask = new tasks.LambdaInvoke(this, `${projectDisplayName}-ValidateInput`, {
    ...
    const startCrawler = new tasks.GlueStartCrawlerRun(this, `${projectDisplayName}-StartProcessedCrawler`, {
      crawlerName: crawler.name!,
    ...
    const logGlueResults = new sfn.Pass(this, `${projectDisplayName}-LogGlueResults`, {
        parameters: {
    ...
    const stateMachine = new sfn.StateMachine(this, `${projectDisplayName}-PipelineStateMachine`, {
      stateMachineName: `FSA-${props.deployEnv}-${projectDisplayName}-Pipeline`,
    ...
    new cdk.CfnOutput(this, `${projectDisplayName}-StateMachineArn`, { value: stateMachine.stateMachineArn });
