  ...
  export class FpacGlueJob extends Construct {
    ...
    constructor(scope: Construct, id: string, props: Props) {
      super(scope, id);
        ...
        const jobName = `FSA-${props.env}-${props.jobType}`;

        this.job = new glue.PySparkEtlJob(this, jobName, {
            jobName: jobName,
            role: props.role,
            script: scriptCode,
            numberOfWorkers: 2,
        ...
        this.workflow = new glue_l1.CfnWorkflow(this, `${jobName}-PipelineWorkflow`, {
           name: `${cdk.Stack.of(this).stackName}-${jobName}-workflow`,
        });

        this.trigger = new glue_l1.CfnTrigger(this, `${jobName}-Trigger`, {
            name: `${cdk.Stack.of(this).stackName}-${jobName}-trigger`,
            type: 'ON_DEMAND',
            actions: [{
